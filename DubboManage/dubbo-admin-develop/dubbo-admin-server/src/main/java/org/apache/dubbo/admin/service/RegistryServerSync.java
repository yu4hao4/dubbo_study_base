...
public class RegistryServerSync implements InitializingBean, DisposableBean, NotifyListener {
    ...
    // Notification of of any cn.service with any type (override、subcribe、route、provider) is full.
    @Override
    public void notify(List<URL> urls) {
        ...
        for (Map.Entry<String, Map<String, Map<String, URL>>> categoryEntry : categories.entrySet()) {
            ...
            } else {// Fix map can not be cleared when cn.service is unregistered: when a unique “group/cn.service:version” cn.service is unregistered, but we still have the same services with different version or group, so empty protocols can not be invoked.
