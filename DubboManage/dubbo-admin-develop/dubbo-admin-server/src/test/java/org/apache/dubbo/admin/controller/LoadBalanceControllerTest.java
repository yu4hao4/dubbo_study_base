...
public class LoadBalanceControllerTest extends AbstractSpringIntegrationTest {
    ...
    public void createLoadbalance() throws IOException {
        ...
        // cn.service and application are all blank
        response = restTemplate.postForEntity(url("/api/{env}/rules/balancing"), balancingDTO, String.class, env);
        assertFalse("should return a fail response, when cn.service and application are all blank", (Boolean) objectMapper.readValue(response.getBody(), Map.class).get("success"));
        // dubbo version is 2.6
        balancingDTO.setApplication("test application");
        balancingDTO.setService("test cn.service");
    ...
    public void searchLoadbalances() throws IOException {
        String service = "test cn.service", application = "test application";
        ResponseEntity<String> response;
        // cn.service and application are all blank
        response = restTemplate.getForEntity(url("/api/{env}/rules/balancing"), String.class, env);
        assertFalse("should return a fail response, when cn.service and application are all blank", (Boolean) objectMapper.readValue(response.getBody(), Map.class).get("success"));
        // cn.service is valid
        response = restTemplate.getForEntity(url("/api/{env}/rules/balancing?cn.service={cn.service}&application={application}"), String.class, env, service, null);
        assertEquals(HttpStatus.OK, response.getStatusCode());
        verify(overrideService).findBalance(service);
        // application is valid
        response = restTemplate.getForEntity(url("/api/{env}/rules/balancing?cn.service={cn.service}&application={application}"), String.class, env, null, application);
        assertEquals(HttpStatus.OK, response.getStatusCode());
