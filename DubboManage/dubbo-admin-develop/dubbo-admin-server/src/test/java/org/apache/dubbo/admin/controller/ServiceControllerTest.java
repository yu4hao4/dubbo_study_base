...
public class ServiceControllerTest extends AbstractSpringIntegrationTest {
  ...
  public void shouldGetAllServices() throws Exception {
    final int num = 10;
    for (int i = 0; i < num; i++) {
      final String service = "org.apache.dubbo.admin.test.cn.service" + i;
      final URL url = i % 2 == 0
  ...
  public void shouldGetAllApplications() throws Exception {
    final int num = 10;
    for (int i = 0; i < num; i++) {
      final String service = "org.apache.dubbo.admin.test.cn.service";
  ...
  public void shouldFilterUsingPattern() throws InterruptedException {
    ...
    for (int i = 0; i < num; i++) {
      final String service = "org.apache.dubbo.admin.test.cn.service" + i + ".pattern" + (i % 2);
    ...
    ResponseEntity<Set<ServiceDTO>> responseEntity = restTemplate.exchange(
        url("/api/{env}/cn.service?pattern={pattern}&filter={filter}"),
