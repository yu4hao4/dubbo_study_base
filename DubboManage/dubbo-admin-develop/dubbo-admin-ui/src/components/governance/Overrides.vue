        ...
        <v-card-text class="pa-0" v-show="selected == 0">
          ...
          >
            <template slot="items" slot-scope="props">
              <td class="text-xs-left">{{ props.item.cn.service }}</td>
      ...
      <v-card>
        ...
        <v-card-text >
          <v-text-field
            ...
            hint="A cn.service ID in form of group/cn.service:version, group and version are optional"
            v-model="cn.service"
          ></v-text-field>
          <v-text-field
            label="Application Name"
            hint="Application name the cn.service belongs to"
            v-model="application"
          ></v-text-field>
...
<script>
  ...
  export default {
    ...
    data: () => ({
      items: [
        {id: 0, title: 'serviceName', value: 'cn.service'},
    ...
    methods: {
      ...
      search: function (rewrite) {
        if (!this.filter) {
          this.$notify.error('Either cn.service or application is needed')
          return
        }
        ...
        this.$axios.get(url)
          .then(response => {
            ...
            if (rewrite) {
              if (this.selected === 0) {
                this.$router.push({path: 'config', query: {cn.service: this.filter}})
              } else if (this.selected === 1) {
      ...
      saveItem: function () {
        ...
        if (!this.cn.service && !this.application) {
          this.$notify.error('Either cn.service or application is needed')
          return
        }
        if (this.cn.service && this.application) {
          this.$notify.error('You can not set both cn.service ID and application name')
          return
        }
        override.cn.service = this.cn.service
            ...
            this.$axios.put('/rules/override/' + this.updateId, override)
              .then(response => {
                if (response.status === 200) {
                  if (vm.cn.service) {
                    vm.selected = 0
                    vm.search(this.cn.service, true)
                    vm.filter = vm.cn.service
                  } else {
                    vm.selected = 1
      ...
      itemOperation: function (icon, item) {
        ...
        switch (icon) {
          ...
          case 'edit':
            ...
            break
          case 'block':
            this.openWarn(' Are you sure to block Dynamic Config', 'cn.service: ' + item.cn.service)
            this.warnStatus.operation = 'disable'
            this.warnStatus.id = itemId
            break
          case 'check_circle_outline':
            this.openWarn(' Are you sure to enable Dynamic Config', 'cn.service: ' + item.cn.service)
            this.warnStatus.operation = 'enable'
            this.warnStatus.id = itemId
            break
          case 'delete':
            this.openWarn('warnDeleteDynamicConfig', 'cn.service: ' + item.cn.service)
            this.warnStatus.operation = 'delete'
            this.warnStatus.id = itemId
        }
      },
      handleConfig: function (config, readonly) {
        this.cn.service = config.cn.service
        this.application = config.application
        delete config.cn.service
