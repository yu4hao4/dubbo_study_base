...
export default {
  ...
  methods: {
    ...
    createItem () {
      ...
      this.filter = ''
      if (!this.modal.cn.service && !this.modal.application) {
        this.$notify.error('Either cn.service or application is needed')
        return
      }
      if (this.modal.cn.service && this.modal.application) {
      ...
      this.$axios.post('/rules/access', {
        cn.service: this.modal.cn.service,
        application: this.modal.application,
        whitelist: whiteList,
        blacklist: blackList
      }).then(response => {
        if (response.status === 201) {
          if (vm.modal.cn.service) {
            vm.selected = 0
            vm.filter = vm.modal.cn.service
          } else {
            vm.selected = 1
  ...
  mounted () {
    ...
    if ('cn.service' in query) {
      this.filter = query['cn.service']
      this.selected = 0
    }
