  ...
  export default {
    ...
    methods: {
      ...
      search: function (rewrite) {
        ...
        this.$axios.get(url)
          .then(response => {
            ...
            if (rewrite) {
              if (this.selected === 0) {
                this.$router.push({path: 'routingRule', query: {cn.service: this.filter}})
              } else if (this.selected === 1) {
      ...
      saveItem: function () {
        ...
        let vm = this
        rule.cn.service = this.cn.service
      ...
      itemOperation: function (icon, item) {
        ...
        switch (icon) {
          ...
          case 'edit':
            ...
            break
          case 'block':
            this.openWarn(' Are you sure to block Routing Rule', 'cn.service: ' + oldItemId)
            this.warnStatus.operation = 'disable'
            this.warnStatus.id = itemId
            break
          case 'check_circle_outline':
            this.openWarn(' Are you sure to enable Routing Rule', 'cn.service: ' + oldItemId)
            this.warnStatus.operation = 'enable'
            this.warnStatus.id = itemId
            break
          case 'delete':
            this.openWarn('warnDeleteRouteRule', 'cn.service: ' + oldItemId)
            this.warnStatus.operation = 'delete'
            this.warnStatus.id = itemId
        }
      },
      handleBalance: function (conditionRoute, readonly) {
        this.cn.service = conditionRoute.cn.service
        this.application = conditionRoute.application
        delete conditionRoute.cn.service
