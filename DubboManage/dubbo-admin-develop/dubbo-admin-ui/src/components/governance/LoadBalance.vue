  ...
  export default {
    ...
    methods: {
      ...
      search: function (rewrite) {
        ...
        this.$axios.get(url)
          .then(response => {
            ...
            if (rewrite) {
              if (this.selected === 0) {
                this.$router.push({path: 'loadbalance', query: {cn.service: this.filter}})
              } else if (this.selected === 1) {
      ...
      saveItem: function () {
        ...
        let vm = this
        balancing.cn.service = this.cn.service
      ...
      itemOperation: function (icon, item) {
        ...
        switch (icon) {
          ...
          case 'edit':
            ...
            break
          case 'delete':
            this.openWarn('warnDeleteBalancing', 'cn.service: ' + itemId)
            this.warnStatus.operation = 'delete'
            this.warnStatus.id = itemId
        }
      },
      handleBalance: function (balancing, readonly) {
        this.cn.service = balancing.cn.service
        this.application = balancing.application
        // delete balancing.cn.service
