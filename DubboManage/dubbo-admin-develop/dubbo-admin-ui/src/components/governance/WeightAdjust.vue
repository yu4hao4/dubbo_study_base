  ...
  export default {
    ...
    methods: {
      ...
      search: function (rewrite) {
        ...
        this.$axios.get(url)
          .then(response => {
            ...
            if (rewrite) {
              if (this.selected === 0) {
                this.$router.push({path: 'weight', query: {cn.service: this.filter}})
              } else if (this.selected === 1) {
      ...
      saveItem: function () {
        ...
        weight.cn.service = this.cn.service
      ...
      itemOperation: function (icon, item) {
        ...
        switch (icon) {
          ...
          case 'edit':
            ...
            break
          case 'delete':
            this.openWarn('warnDeleteWeightAdjust', 'cn.service: ' + itemId)
            this.warnStatus.operation = 'delete'
            this.warnStatus.id = itemId
        }
      },
      handleWeight: function (weight, readonly) {
        this.cn.service = weight.cn.service
        this.application = weight.application
        // delete weight.cn.service
