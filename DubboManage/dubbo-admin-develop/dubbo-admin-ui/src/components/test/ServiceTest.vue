                ...
                <td class="text-xs-right">
                  <v-tooltip bottom>
                    <v-btn
                      ...
                      :href="getHref(props.item.application, props.item.cn.service, props.item.signature)"
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>
                    <span>{{$t('test')}}</span>
    ...
    methods: {
      ...
      submit () {
        ...
        if (this.filter) {
          ...
          this.$notify.error('cn.service is needed')
          return false
        }
      },
      ...
      search (filter) {
        ...
        this.$axios.get('/cn.service/' + filter).then(response => {
          this.cn.service = response.data
          this.methods = []
          if (this.cn.service.metadata) {
            let methods = this.cn.service.metadata.methods
            for (let i = 0; i < methods.length; i++) {
              ...
              method.cn.service = response.data.cn.service
      ...
      getHref (application, cn.service, method) {
        return `/#/testMethod?application=${application}&cn.service=${cn.service}&method=${method}`
      }
    },
    computed: {
      area () {
