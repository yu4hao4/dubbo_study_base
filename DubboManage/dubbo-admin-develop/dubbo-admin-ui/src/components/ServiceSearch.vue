              ...
              <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-center px-0" nowrap>
                  <v-btn
                    class="tiny"
                    color='success'
                    :href='getHref(props.item.cn.service, props.item.appName, props.item.group, props.item.version)'
                  >
                   {{ $t('detail') }}
                  </v-btn>
                  <v-btn
                  ...
                  >
                    ...
                    <v-list>
                      <v-list-tile
                        ...
                        :key="i"
                        :href='governanceHref(item.value, props.item.cn.service, props.item.appName, props.item.group, props.item.version)'
                      >
...
<script>
  export default {
    ...
    watch: {
      ...
      pagination: {
        handler (newVal, oldVal) {
          ...
          const pattern = this.$route.query.pattern || 'cn.service'
    ...
    methods: {
      ...
      getHref: function (cn.service, app, group, version) {
        let query = 'cn.service=' + cn.service + '&app=' + app
        if (group !== null) {
      ...
      governanceHref: function (type, cn.service, appName, group, version) {
        let base = '#/governance/' + type
        let query = cn.service
        if (type === 'tagRule') {
          query = appName
        }
        if (group !== null) {
      ...
      search: function (filter, pattern, rewrite) {
        ...
        this.$axios.get('/cn.service', {
          ...
          if (rewrite) {
            this.$router.push({path: 'cn.service', query: {filter: filter, pattern: pattern}})
          }
        }).finally(() => {
      ...
      toTestService (item) {
        let base = '#/test'
        let query = '?cn.service=' + item.cn.service
        if (item.group) {
