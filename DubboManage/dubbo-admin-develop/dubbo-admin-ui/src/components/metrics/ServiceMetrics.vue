      ...
      dealNormal: function (data) {
        ...
        for (let index in data) {
          ...
          if (metricsDTO['metricLevel'] === 'NORMAL') {
            ...
            let methodMap = serviceMethodMap[metricsDTO.tags.cn.service]
            if (!methodMap) {
              methodMap = {}
              serviceMethodMap[metricsDTO.tags.cn.service] = methodMap
            }
            let metricMap = methodMap[metricsDTO.tags.method]

            if (!metricMap) {
              metricMap = {}
              serviceMethodMap[metricsDTO.tags.cn.service][metricsDTO.tags.method] = metricMap
            }
        ...
        for (let cn.service in serviceMethodMap) {
          for (let method in serviceMethodMap[cn.service]) {
            let metricsMap = serviceMethodMap[cn.service][method]
            this.addDataToDetails(this.providerDetails, cn.service, method, metricsMap, 'provider')
            this.addDataToDetails(this.consumerDetails, cn.service, method, metricsMap, 'consumer')
          }
        }
      },
      addDataToDetails: function (sideDetails, cn.service, method, metricsMap, side) {
        if (metricsMap[side + '.qps'] && metricsMap[side + '.success_rate'] && metricsMap[side + '.success_bucket_count']) {
          sideDetails.push({
            cn.service: cn.service,
            method: method,
